package com.example.ms_subscriptions.exception;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleSubscriptionNotFound(SubscriptionNotFoundException ex) {
        log.warn("Suscripción no encontrada: {}", ex.getMessage());
        ErrorResponse error = new ErrorResponse(404, "Suscripción no encontrada", ex.getMessage());
        return ResponseEntity.status(HttpStatus.NOT_FOUND).body(error);
    }

    @ExceptionHandler(InvalidSubscriptionTypeException.class)
    public ResponseEntity<ErrorResponse> handleInvalidType(InvalidSubscriptionTypeException ex) {
        log.warn("Tipo de suscripción inválido: {}", ex.getMessage());
        ErrorResponse error = new ErrorResponse(400, "Tipo de suscripción inválido", ex.getMessage());
    ...
    public ResponseEntity<ErrorResponse> handleAlreadyActive(SubscriptionAlreadyActiveException ex) {
        log.warn("Conflicto - Suscripción ya activa: {}", ex.getMessage());
    ...
    public ResponseEntity<ErrorResponse> handleNotActive(SubscriptionNotActiveException ex) {
        log.warn("Suscripción inactiva: {}", ex.getMessage());
        ErrorResponse error = new ErrorResponse(400, "Suscripción inactiva", ex.getMessage());
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(error);
    }
